...
public class GroovyConsoleHelper {
    ...
    public static String getScriptCustomFormElements(String scriptURI, HttpServletRequest request) {
        if (StringUtils.isBlank(scriptURI)) return StringUtils.EMPTY;
        final StringBuilder sb = new StringBuilder();
        try {
            final UrlResource resource = new UrlResource(StringUtils.substringBeforeLast(scriptURI, ".groovy") + ".properties");
            if (resource.exists()) {
                final Properties confs = new Properties();
                confs.load(resource.getInputStream());
                final String[] paramNames = StringUtils.split(confs.getProperty("script.parameters.names", "").replaceAll("\\s", ""), ",");
        ...
        if (StringUtils.isBlank(formElements)) return StringUtils.EMPTY;
        return sb.delete(0, sb.length()).append("<fieldset><legend>Script configuration</legend>").append(formElements).append("</fieldset>").toString();
    }

    private static void generateFormElement(String paramName, StringBuilder sb, Properties confs, HttpServletRequest request) {
        final String paramType = confs.getProperty(String.format("script.param.%s.type", paramName), "checkbox").trim();
        if ("checkbox".equals(paramType)) generateCbFormElement(paramName, sb, confs, request);
        else if ("text".equals(paramType)) generateTextFormElement(paramName, sb, confs, request);
        else
            logger.error(String.format("Unsupported form element type form the parameter %s: %s", paramName, paramType));
    }

    private static void generateCbFormElement(String paramName, StringBuilder sb, Properties confs, HttpServletRequest request) {
        ...
        sb.append("<input type=\"checkbox\" name=\"scriptParam_").append(paramName).append("\" id=\"scriptParam_").append(paramName);
        ...
        if (StringUtils.isNotBlank(paramVal) && ("on".equalsIgnoreCase(paramVal.trim()) || "true".equalsIgnoreCase(paramVal.trim())))
            sb.append("\" checked=\"true");
        sb.append("\" /></p>");
    }

    private static void generateTextFormElement(String paramName, StringBuilder sb, Properties confs, HttpServletRequest request) {
        sb.append("<p><label for=\"scriptParam_").append(paramName).append("\">");
        sb.append(confs.getProperty(String.format("script.param.%s.label", paramName), paramName)).append("</label> ");
        sb.append("<input type=\"text\" name=\"scriptParam_").append(paramName).append("\" id=\"scriptParam_").append(paramName);
        final String paramVal;
        if ("true".equals(request.getParameter("runScript"))) {
            paramVal = request.getParameter("scriptParam_" + paramName);
        } else {
        ...
        if (StringUtils.isNotBlank(paramVal)) sb.append("\" value=\"").append(paramVal);
        sb.append("\" /></p>");
    }
