...
<c:set var="title" value="JCR Browser"/>
...
<c:set var="nodeId"
       value="${not empty param.uuid ? fn:trim(fn:escapeXml(param.uuid)) : 'cafebabe-cafe-babe-cafe-babecafebabe'}"/>
<c:set var="showJCRNodes"
       value="${not empty param.showJCRNodes ? fn:trim(fn:escapeXml(param.showJCRNodes)) : 'false'}"/>
<%!
...
<div class="${workspace}">
    <fieldset>
        ...
        &nbsp;<a href="#go"
                 onclick='var path=document.getElementById("goToPath").value; if (path.length > 0) { go("path", path); } return false;'
                 title="Go to the node with path"
    ><img src="<c:url value='/icons/refresh.png'/>" height="16" width="16" title="Go to the node with path" border="0"
        ...
        &nbsp;<a href="#go"
                 onclick='var uuid=document.getElementById("goToUuid").value; if (uuid.length > 0) { go("uuid", uuid); } return false;'
                 title="Go to the node with UUID"><img src="<c:url value='/icons/search.png'/>" height="16" width="16"
                                                       title="Go to the node with UUID" border="0"
                                                       style="vertical-align: middle;"/></a>
    </fieldset>
    ...
    <fieldset>
        ...
        <legend><strong>This is the ${workspace} workspace</strong>&nbsp;(<a href="#switchWorkspace"
                                                                             onclick="go('workspace', '${anotherWorkspace}'); return false;">switch
            to ${anotherWorkspace}</a>)
            &nbsp; browse as ${showJCRNodes?"<span style='color:red;font-weight:bold'>jcr</span>":"jahia"} session (<a
                    href="#showJCRNodes" onclick="go('showJCRNodes','${showJCRNodes?"false":"true"}'); return false;">switch
                to ${!showJCRNodes?"jcr":"jahia"}</a>) )
            &nbsp;
            <a href="${mgrUrl}" target="_blank"><img src="<c:url value='/icons/fileManager.png'/>" width="16"
                                                     height="16" alt=" " role="presentation"
                                                     style="position:relative; top: 4px; margin-right:2px;"/>repository
                explorer</a>
        </legend>
        ...
        <fieldset style="position: absolute; right: 20px;">
            ...
            <p>
                <input id="cbActions" type="checkbox" ${showActions ? 'checked="checked"' : ''}
                       onchange="go('showActions', '${!showActions}')"/>&nbsp;<label for="cbActions">Show
                actions</label><br/>
                <input id="cbProperties" type="checkbox" ${showProperties ? 'checked="checked"' : ''}
                       onchange="go('showProperties', '${!showProperties}')"/>&nbsp;<label for="cbProperties">Show
                properties</label><br/>
                <input id="cbNodes" type="checkbox" ${showNodes ? 'checked="checked"' : ''}
                       onchange="go('showNodes', '${!showNodes}')"/>&nbsp;<label for="cbNodes">Show child
                nodes</label><br/>
                <input id="cbSortNodes" type="checkbox" ${sortNodes ? 'checked="checked"' : ''}
                       onchange="go('sortNodes', '${!sortNodes}')"/>&nbsp;<label for="cbSortNodes">Sort child
                nodes</label><br/>
                <input id="cbReferences" type="checkbox" ${showReferences ? 'checked="checked"' : ''}
                       onchange="go('showReferences', '${!showReferences}')"/>&nbsp;<label for="cbReferences">Show
                references</label><br/>
                <input id="cbVersions" type="checkbox" ${showVersions ? 'checked="checked"' : ''}
                       onchange="go('showVersions', '${!showVersions}')"/>&nbsp;<label for="cbVersions">Show
                versioning</label><br/>
            </p>
        </fieldset>
            ...
            <c:choose>
                <c:when test="${param.action == 'delete' && not empty param.target}">
                    <% Node target = jcrSession.getNodeByIdentifier(request.getParameter("target"));
                        ...
                        jcrSession.save();
                    %>
                    <p style="color: blue">Node <strong>${fn:escapeXml(deletedTargetPath)}</strong> deleted successfully
                    </p>
                </c:when>
                <c:when test="${param.action == 'rename' && not empty param.target && not empty param.value}">
                    <% Node target = jcrSession.getNodeByIdentifier(request.getParameter("target"));
                        ...
                        jcrSession.save();
                    %>
                    <p style="color: blue">Node <strong>${fn:escapeXml(deletedTargetPath)}</strong> renamed successfully
                    </p>
                </c:when>
                ...
                <c:when test="${param.action == 'unlockTree'}">
                    <%
                        if (node instanceof JCRNodeWrapper && jcrSession instanceof JCRSessionWrapper) {
                            ...
                            jcrSession.save();
                    %>
                    <p style="color: blue">Locks cleared for node <strong>${fn:escapeXml(node.path)}</strong> and its
                        children</p>
                    <%
                        }
                    %>
                ...
                <c:when test="${param.action == 'reindex-tree'}">
                    ...
                    %>
                    <p style="color: blue">Re-indexing tree in workspace <strong>${workspace}</strong>, starting from
                        node <strong>${nodeId}</strong> has been completed in
                        <strong><%= System.currentTimeMillis() - treeReindexStartTime %>
                        </strong> ms</p>
                    <%
                        }
                    %>
        ...
        <p>
            <c:if test="${!node.locked}">
                <img src="<c:url value='/icons/lock.png'/>" height="16" width="16" border="0"
                     style="vertical-align: middle;"
                     alt=" "/>&nbsp;<a href="#lock" onclick="if (confirm('You are about to put a lock on this node. Continue?')) {go('action', 'lock');} return false;" title="Put a lock on this node">lock node</a>
            </c:if>
            <c:if test="${node.locked}">
                <img src="<c:url value='/icons/unlock.png'/>" height="16" width="16" border="0"
                     style="vertical-align: middle;"
                     alt=" "/>&nbsp;<a href="#unlock" onclick="if (confirm('You are about to remove all locks on this node. Continue?')) {go('action', 'unlock');} return false;" title="Clean all locks on this node">unlock node</a>
                <img src="<c:url value='/icons/unlock.png'/>" height="16" width="16" border="0"
                     style="vertical-align: middle;"
                     alr=" "/>&nbsp;<a href="#unlockTree" onclick="if (confirm('You are about to remove all locks on this node and its children. Continue?')) {go('action', 'unlockTree');} return false;" title="Clean all locks on this node and its children">unlock tree</a>
            </c:if>
            <% if (node instanceof JCRNodeWrapper && ((JCRNodeWrapper) node).getProvider().isDefault()) { %>
            &nbsp;
            <img src="<c:url value='/icons/reversePublish.png'/>" height="16" width="16" border="0"
                 style="vertical-align: middle;" alt=" "/>&nbsp;<a href="#reindex-tree"
                                                                   onclick="if (confirm('This will execute (synchronously) a re-indexing of the JCR sub-tree in the ${workspace} workspace, starting with the this node. Would you like to continue?')) {go('action', 'reindex-tree');} return false;"
                                                                   title="Re-index the node and its whole sub-tree in ${workspace} workspace">re-index
            node and sub-tree</a>
            <% } %>
        </p>
        </c:if>
        ...
        <strong>Mixins:&nbsp;</strong>[<c:forEach items="${node.mixinNodeTypes}" var="mixin"
                                                  varStatus="status">${status.index > 0 ? ", " : ""}${mixin.name}<c:if
            test="${showActions}">&nbsp;<a href="#remove" onclick="if (confirm('You are about to remove mixin ${mixin.name} from the node. Continue?')) {go('action', 'removeMixin', 'value', '${mixin.name}');} return false;"><img src="<c:url
            value='/icons/delete.png'/>" height="16" width="16" title="Delete mixin" border="0" style="vertical-align: middle;"/></a></c:if></c:forEach>]<br/>
        <c:if test="${showActions}">
            <%
            ...
            <button onclick="var newMixin=document.getElementById('mixins').value; if (confirm('You are about to add mixin ' + newMixin + ' to the node. Continue?')) {go('action', 'addMixin', 'value', newMixin);} return false;">
                add
            </button>
        </c:if>
        <%
        ...
        <c:if test="${jcr:isNodeType(node, 'nt:file')}">
            <br/><strong>File:&nbsp;</strong><a target="_blank" href="<c:url value='${node.url}' context='/'/>"
                                                title="download"><img src="<c:url value='/icons/download.png'/>"
                                                                      height="16" width="16" title="download" border="0"
                                                                      style="vertical-align: middle;"/></a>
        </c:if>
        <%
            }
        ...
        <p><strong>Properties:&nbsp;</strong><a href="#properties"
                                                onclick="go('showProperties', ${showProperties ? 'false' : 'true'}); return false;">${showProperties ? 'hide' : 'show'}</a>
        </p>
                            ...
                            <c:if test="${property.multiple}" var="multiple">
                                <ul>
                                    ...
                                    <c:forEach items="${property.values}" var="value">
                                        <li>
                                            <%@include file="commons/value.jspf" %>
                                        </li>
                                    </c:forEach>
                                </ul>
                            </c:if>
        ...
        <p><strong>References:&nbsp;</strong><a href="#references"
                                                onclick="go('showReferences', ${showReferences ? 'false' : 'true'}); return false;">${showReferences ? 'hide' : 'show'}</a>
        </p>
        <c:if test="${showReferences}">
            <ul>
                <%try {%>
                <c:set var="refsCount"
                       value="${functions:length(node.references) + functions:length(node.weakReferences)}"/>
                ...
                <%} catch (Exception ex) {%>
                <p style="color:red;">Error retrieving
                    references<br/>Cause: <%=(ex.getCause() != null ? ex.getCause().toString() : ex.toString())%>
                </p>
                <%} %>
            </ul>
        </c:if>

        <p><strong>Child nodes:&nbsp;</strong><a href="#nodes"
                                                 onclick="go('showNodes', '${!showNodes}'); return false;">${showNodes ? 'hide' : 'show'}</a>
            <c:if test="${showNodes}">
            / <a href="#sortNodes"
                 onclick="go('sortNodes', '${!sortNodes}'); return false;">${sortNodes ? 'no sort' : 'sort'}</a>
        ...
        <ul>
            ...
            <c:if test="${childrenCount > 0}">
                ...
                <c:forEach items="${not empty sortedNodes ? sortedNodes : nodes}" var="child">
                    ...
                    <li>
                        <a href="#child"
                           onclick="go('uuid', '${child.identifier}'); return false;">${fn:escapeXml(child.name)}</a>&nbsp;(${childNodeTypes})
                        <c:if test="${showActions}">
                            &nbsp;|
                            <%
                            ...
                            <c:if test="${jcr:isNodeType(child, 'nt:file')}">
                                &nbsp;<a target="_blank" href="<c:url value='${child.url}'
                                                                      context='/'/>" title="download"><img src="<c:url
                                    value='/icons/download.png'/>" height="16" width="16" title="download" border="0" style="vertical-align: middle;"/></a>
                            </c:if>
                            <%
                                }
                            %>
                            ...
                            &nbsp;<a target="_blank" href="${fn:escapeXml(urlExportXml)}" title="Export as XML"><img src="<c:url
                                value='/icons/import.png'/>" height="16" width="16" title="Export as XML" border="0" style="vertical-align: middle;"/></a>
                            &nbsp;<a target="_blank" href="${fn:escapeXml(urlExportZip)}" title="Export as ZIP"><img src="<c:url
                                value='/icons/zip.png'/>" height="16" width="16" title="Export as ZIP" border="0" style="vertical-align: middle;"/></a>
                            |&nbsp;
                            <c:set var="childNameEscaped"
                                   value="${fn:escapeXml(functions:escapeJavaScript(child.name))}"/>
                            &nbsp;<a href="#rename" onclick="var name=prompt('Please provide a new name for the node:', '${childNameEscaped}'); if (name != null & name != '${childNameEscaped}') { go('action', 'rename', 'target', '${child.identifier}', 'value', name);} return false;" title="Rename"><img src="<c:url
                                value='/icons/editContent.png'/>" height="16" width="16" title="Rename" border="0" style="vertical-align: middle;"/></a>
                            &nbsp;<a href="#delete" onclick="var nodeName='${childNameEscaped}'; if (!confirm('You are about to delete the node ' + nodeName + ' with all child nodes. Continue?')) return false; go('action', 'delete', 'target', '${child.identifier}'); return false;" title="Delete"><img src="<c:url
                                value='/icons/delete.png'/>" height="16" width="16" title="Delete" border="0" style="vertical-align: middle;"/></a>
                        </c:if>
                    </li>
                </c:forEach>
            </c:if>
        ...
        <c:if test="${showVersions}">
            <strong>Linear history:&nbsp;</strong>[<c:forEach items="${versionIterator}" var="version"
                                                              varStatus="status">${status.index > 0 ? ", " : ""}<a
                href="#version"
                onclick="go('uuid', '${version.identifier}'); return false;">${version.name}</a></c:forEach>]<br>
